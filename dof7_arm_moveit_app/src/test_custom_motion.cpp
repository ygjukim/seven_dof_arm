...
int main(int argc, char **argv)
{
  ...
  moveit::core::RobotStatePtr robot_state_ = visual_tools_->getSharedRobotState();
  const moveit::core::JointModelGroup* jmg_ = robot_state_->getJointModelGroup(PLANNING_GROUP);
  ...
  ROS_INFO_NAMED("tutorial", "Visualizing plan 1 (pose goal) %s", success ? "SUCCEEDED" : "FAILED");
  ...
  visual_tools_->publishTrajectoryLine(my_plan.trajectory_, jmg_);
  visual_tools_->trigger();
//  visual_tools_->prompt("next step");
  ...
  moveit::core::RobotStatePtr current_state = visual_tools_->getSharedRobotState();
  //
  // Next get the current set of joint values for the group.
//  std::vector<double> joint_group_positions;
//  current_state->copyJointGroupPositions(jmg_, joint_group_positions);
  static const double arr[] = 
    { -2.19642133528, 1.01765315966, 0.705631162427, 
      -0.34480897755, -0.911004391473, -0.854381228205, 2.48564693549};
  std::vector<double> joint_group_positions(arr, arr+sizeof(arr)/sizeof(arr[0]));
